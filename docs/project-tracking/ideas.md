...
- **Shipped (v0.20.0, 2026-08-17) — idea COMPLETE:** `--refresh` on `stamp-portable-layer.sh`, wired through as `/make-portable refresh` with a confirm gate; scoped to the two plugin-owned files (vendored validator + operator's manual), never `AGENTS.md`, never facts/index. See `resolved.md` A-20260817-portable-layer-refresh. Decision: D-20260817-portable-refresh-scope (why AGENTS.md is excluded, and why "refresh only if unmodified" is unimplementable without provenance tracking).
