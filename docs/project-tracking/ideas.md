...
- To start, future-us needs: a schema decision (enum vs verified-against vs both) in `conventions/memory.md` frontmatter, then `memory_graph.py` / `/memory-lint` support to sort and report by it. Relates to stale-priors-memory (shipped), [[integrity-auditor]], [[memory-backlinks-search]], SP2-memory (shipped).
...
- To start, future-us needs: add optional `applies-to:` to `conventions/memory.md` frontmatter (default = repo), and decide whether `/memory-lint` surfaces it; no index change. Relates to SP2-memory (shipped), [[memory-volatility-field]].
