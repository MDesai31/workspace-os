...
- **Shipped (2026-07-01):** the generic command/content guardrail engine (`hooks/guardrail.sh`) + per-repo `.claude/guardrails.json` (declarative `bash`/`write` deny/warn rules) + warn-only universal defaults + `templates/guardrails.json`. Registered globally via plugin `hooks.json` (no per-repo `settings.json` wiring). See `resolved.md` A-20260701-guardrail-engine; decision D-20260701-guardrail-engine. **Remaining sub-slice:** the PostToolUse advisory-lint template (`ruff`/`eslint`, language-parameterized) — separate hook, deferred.
