...
- **Remaining sub-slices:** (a) foreign memory-format conversion (top-level `memory/`, wiki → workspace-os schema); (b) roadmap/TODO → tracking (prior tracking docs → action-items/ideas/decisions); (c) resolve CLAUDE.md `@imports` during scan — the klapp dogfood found imported files (e.g. `@AGENTS.md`) are invisible to classification though they're effectively always-loaded; (d) widen/parameterize candidate globs — `AGENTS.md`/`RUNNING.md` aren't scanned unless passed as the path argument.
