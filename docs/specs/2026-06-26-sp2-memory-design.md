...
- **SP2b (round-out):** `/memory-lint` + `/memory-sync` + warn-only secret hook.
...
| `/ingest`, `/memory-lint`, `/memory-sync` + guard hooks | `/ingest`, `/memory-lint`, `/memory-sync` + **warn-only** hook |
...
- `/memory-sync` moves a named `~/.claude` fact into a repo's `docs/memory/` with confirmation, refusing on detected secrets (warn).
