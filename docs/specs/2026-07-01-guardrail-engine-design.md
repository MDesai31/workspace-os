...
hooks/guardrail.sh   (engine — plugin, portable, updated centrally)
  ...
  └─ deny → exit 2 + reason on stderr  |  warn → {"systemMessage": reason} JSON on stdout, exit 0
...
- `action: "deny"` → **exit code 2** with `reason` on stderr (PreToolUse blocking contract — the
  reason is surfaced to Claude). `action: "warn"` → a top-level `{"systemMessage": "<reason>"}` JSON
  object on **stdout**, **exit 0** (the documented user-facing warning channel; non-blocking).
  Warns deliberately do **not** use `additionalContext` — that would inject into Claude's context on
  routine edits (e.g. the broad secret-content default), which we don't want.
...
all matched deny reasons on stderr). Otherwise all matching `warn` reasons are joined into one
`systemMessage` JSON object on stdout and the engine exits 0.
