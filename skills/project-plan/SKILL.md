...
`conventions/project-tracking.md`. Writes to `docs/project-tracking/ideas.md` (run `/project-init`
first if tracking doesn't exist).
...
2. **Tag the workstream.** Pick one tag from `docs/project-tracking/README.md`'s `## Workstreams`
   list (ask if unclear).
